...

	{
		...
		for (unsigned int j = 0; j < m_model->numberOfNeighbors(i); j++)
...

			{
				...
				sum += m_model->getMass(neighborIndex) * (m_simulationData.getDij_pj(i) - m_simulationData.getDii(neighborIndex)*

				m_simulationData.getLastPressure(neighborIndex) - (d_jk_pk - d_ji_pi)).dot(kernel);
